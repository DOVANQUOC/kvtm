...
{$x0=0;};
            ...
            $sql = "UPDATE control SET Den2 = $x0"; // Update present status to database
...
{$x1=0;};
            ...
            $sql = "UPDATE control SET Bom2 = $x1"; // Update present status to database
            // Echo "0" , equivalent with send data to App to toast OFF
            if ($conn->query($sql) === TRUE) {  // Because it's been a long time , so i forgot
                //$_GET[] = 0;                      // why i have to put this line but it still run when it's commented
            } 
...
{$x2=0;};
            ...
            $sql = "UPDATE control SET Ps2 = $x2";  // Update present status to database
            // Echo "0" , equivalent with send data to App to toast OFF
            if ($conn->query($sql) === TRUE) {  // Because it's been a long time , so i forgot
                //$_GET[] = 0;                      // why i have to put this line but it still run when it's commented
            } 
...
{$x3=0;};
            ...
            $sql = "UPDATE control SET Rc2 = $x3";  // Update present status to database
            // Echo "0" , equivalent with send data to App to toast OFF
            if ($conn->query($sql) === TRUE) {  // Because it's been a long time , so i forgot
                //$_GET[] = 0;                      // why i have to put this line but it still run when it's commented
            } 
...
/*su dung cho nut nhan chon che do*/    
    ...
    $result = $conn->query($query);
        ...
        while($row = $result->fetch_assoc()) 
        {
        $mode= $row["Mode2"];                    // Echo data , equivalent with send data to esp
        }
///
...
{$mode=0;};
            ...
            $sql = "UPDATE mode SET Mode2 = $mode";  // Update present status to database
            // Echo "0" , equivalent with send data to App to toast OFF
            if ($conn->query($sql) === TRUE) {  // Because it's been a long time , so i forgot
                //$_GET[] = 0;                      // why i have to put this line but it still run when it's commented
            } 
...
<body>
    ...
    <div id="sidebar-wrapper">
        ...
        <ul class="sidebar-nav">
            ...
            <li>
                <a href="schedule2/settime2.php"><span class="fas fa-history" style="color:#FFF8DC;font-size:   15px;"></span>&nbsp; <?php   echo $lang1['schedule']?></a>
            </li>
...
<button style="width:90px;height:30px;color: white;" type="submit" class="btn btn-success btn-sm" name = "ON1" id="chonchedo">auto</button>
</form>
<br><br><br>
                    <div class="hide" id="auto">
                     ...
                     <div class="input-group pull-right"> 
                        ...
                        <form class="box" action="database/submit2.php" method="post" >
                        <input id="
